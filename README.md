...
> **Latest Release: v7.21** — [GWC-Pro-v7.21.zip](https://github.com/QiuSui1145/GWC-Pro/releases/tag/v7.21)
>
> Source distribution (~51 MB). Extract and run `启动全栈环境.bat`.
> Includes prebuilt frontend + Electron deskpet + bundled MMD engine.
> Python runtime, TTS models, ASR models, MMD 3D models, and Live2D models are
> downloaded/imported on first run (see Quick Start).
...
**MMD 3D 桌宠增强：**
- 🎥 完整 MMD 演出包支持：骨骼动画 + 镜头 VMD + 音频 (.wav) 同步播放
- 🎬 镜头 VMD 解析与后台驱动，不干扰桌宠正交渲染
- 🔊 Web Audio API 音频播放，与骨骼动画实时同步
- 🦴 托盘菜单新增骨骼物理开关，双向同步设置面板
- 📁 共享动作库 `mmd_models/_motions/` 支持子目录演出包（自动配对镜头/音频）

**稳定性修复：**
- 🔧 WebGL 上下文不复毁（修复 Live2D↔MMD 切换后模型不渲染）
- 📍 MMD 模型位置按模型路径分别保存（切换模型不再飞出屏幕）
- 🔄 托盘菜单新增「重启桌宠」选项（自动杀旧 cmd 窗口）
- 🛡️ 全栈 Token 认证 + Cookie 同步 + 路径穿越防护 + CORS 收敛
