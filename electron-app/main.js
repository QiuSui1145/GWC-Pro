...
function buildTrayMenu() {
    ...
    tray.setContextMenu(Menu.buildFromTemplate([
        { label: '打开前端界面 (Electron)', click: () => {
            try {
                const desktopDir = path.join(__dirname, '..', 'desktop-app')
                let electronExe = path.join(desktopDir, 'node_modules', 'electron', 'dist', 'electron.exe')
                if (!fs.existsSync(electronExe)) electronExe = path.join(__dirname, 'node_modules', 'electron', 'dist', 'electron.exe')
                if (fs.existsSync(electronExe)) exec(`start "" "${electronExe}" "${desktopDir}"`)
                else require('electron').shell.openExternal('http://127.0.0.1:5202/app/')
            } catch (e) {}
        }},
