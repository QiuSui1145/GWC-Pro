...
async function callLLM(messages) {
    ...
    try {
        const r = await fetch(`${BACKEND_URL}/v1/chat/completions`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ messages, stream: false }) })
...
async function sendMsg(text, atts, inputEl) {
    ...
    try {
        ...
        const reply = await callLLM([{ role: 'user', content }])
        if (reply) { addMessage('assistant', reply); fetch(`${BACKEND_URL}/api/tts_from_pet`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ text: reply }) }).catch(() => {}); if (model) model.motion('Tap') }
