...
rem Read launcher config: when "disable OpenCode autostart" is ON in settings,
rem skip both auto-install and auto-launch (tray menu can still open it).
set "SKIP_OPENCODE="
if exist "userdata\launcher_config.json" (
    findstr /I /C:"\"disableOpencodeAutostart\": true" "userdata\launcher_config.json" >nul 2>nul
    if not errorlevel 1 set "SKIP_OPENCODE=1"
)

echo [0] Checking OpenCode...
if defined SKIP_OPENCODE (
...
if defined SKIP_OPENCODE (
    ...
    if errorlevel 1 (
        ...
        rem GWC_ROOT ends with a backslash, so "%GWC_ROOT%" expands to "...End\".
        rem Inside nested quotes a trailing \" can be read as an escaped quote by
        rem some shells; the trailing dot sidesteps that. (Defensive only - verified
        rem both forms work here; the real cause of the earlier failure was LF line
        rem endings, which this file must never have. Keep it CRLF.)
